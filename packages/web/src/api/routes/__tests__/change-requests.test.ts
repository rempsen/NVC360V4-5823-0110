...
async function seedBooking(opts: {
  ...
  await s.execute({ sql: "DELETE FROM booking_change_requests WHERE booking_id = ?", args: [opts.id] });
  const sched =
...
async function requestRows(bookingId: string) {
  const r = await sqlClient().execute({
    sql: "SELECT * FROM booking_change_requests WHERE booking_id = ? ORDER BY created_at",
    args: [bookingId],
  });
...
beforeAll(async () => {
  ...
  for (const t of [
    ...
    schema.bookingChangeRequests, schema.jobEvents, schema.invoices, schema.properties,
    schema.serviceZones, schema.notificationRules, schema.notifications,
    schema.notificationDeliveries, schema.notificationChannels, schema.webhookEndpoints,
    schema.automationRules, schema.reviews, schema.trackingPings, schema.memberships,
    schema.auditLog,
  ]) {
...
describe("POST /bookings/:id/reschedule", () => {
  ...
  it("is blocked on a booking with no appointment time rather than guessing", async () => {
